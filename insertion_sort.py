...
def insertionSort(arr):
    for i in range(1, len(arr)):
        ...
        while j >= 0 and key < arr[j]:
            arr[j + 1] = arr[j]
            j -= 1
            print(f"{' '.join(Back.RED + str(x) + Back.RESET if i == j or i == j + 1 else str(x) for i, x in enumerate(arr))}")  # Подсвечиваем j и j+1
            time.sleep(0.1)
        arr[j + 1] = key
        print(f"{' '.join(Back.RED + str(x) + Back.RESET if i == j + 1 else str(x) for i, x in enumerate(arr))}")  # Подсвечиваем j+1
        time.sleep(0.1)
