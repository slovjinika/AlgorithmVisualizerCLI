    ...
    for i in range(n):
        for j in range(i + 1, n):
            ...
            if array[i] > array[j]:
                ...
                swaps += 1
                print(f"{' '.join(Back.RED + str(x) + Back.RESET if i == j or i == j else str(x) for i, x in enumerate(array))}")  # Подсвечиваем активные элементы
                time.sleep(0.1)
...
total_comparisons, total_swaps = library_sort(array)
end_time = time.time()
